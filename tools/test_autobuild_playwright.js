...
(async () => {
  ...
  // Check what CPU and GPU were selected on the main stage
  const cpuName = await page.locator('#slot-cpu .slot-item-title').innerText();
  const gpuName = await page.locator('#slot-gpu .slot-item-title').innerText();
  const mbName = await page.locator('#slot-motherboard .slot-item-title').innerText();
  const ramName = await page.locator('#slot-ram .slot-item-title').innerText();
  const ssdName = await page.locator('#slot-ssd .slot-item-title').innerText();

  console.log('\n--- 5000 zł AUTO-BUILD RESULT ---');
  ...
  // Now test 7500 zł
  console.log('\n5. Testing 7500 zł Auto-Builder...');
  await page.click('#btn-auto-builder');
  await page.waitForTimeout(400);
  ...
  const cpu7500 = await page.locator('#slot-cpu .slot-item-title').innerText();
  const gpu7500 = await page.locator('#slot-gpu .slot-item-title').innerText();
