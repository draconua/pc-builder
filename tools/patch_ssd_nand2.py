...
def inject_nand(match):
    ...
    if name_match:
        ...
        # Now update the specs string to include the NAND type
        specs_match = re.search(r"specs:\s*'([^']+)'", full_line)
        if specs_match:
            specs = specs_match.group(1)
            # If we haven't already added it to specs
            if nand not in specs:
                new_specs = specs + f", {nand}"
                full_line = full_line.replace(f"specs: '{specs}'", f"specs: '{new_specs}'")
...
if ssd_match:
    ...
    with open('js/data.js', 'w', encoding='utf-8') as f:
        f.write(js)
    print("Injected nandType into SSDs and updated specs string.")
else:
